...
class TodoApp extends React.Component {
  ...
  componentDidMount() {
    // Loads result from Async into state i.e. previous saved tasks
    loadData("TodoAppState").then(result => this.setState(result));
  }

  componentDidUpdate() {
    // Save current tasks to Async
    saveData("TodoAppState", this.state);
  }

  addTodoItem = task => {
    // Takes in a task, adds to list, updates state
  ...
  removeTodoItem = task => {
    // Takes in a task, removes from list, updates state
    const midList = this.state.dataList.slice();
  ...
  changeCompleted = task => {
    // Takes in a task, flips completed-prop, updates state
    const midList = this.state.dataList.slice();
    const taskIndex = midList.indexOf(task);
    let midTask = task;
  ...
  handleDelete = task => {
    // Fires on pressing delete-button in ListObject
    this.removeTodoItem(task);
  };

  handleChange = task => {
    // Fires on pressing complete-button in ListObject
    this.changeCompleted(task);
  };

  handleAdd = task => {
    // Fires on pressing add-button or pressing Enter in InputBox
  ...
  render() {
    ...
    renderLists = () => {
      // Help function for rendering tasks. If no tasks in dataList, a placeholder text is rendered instead
